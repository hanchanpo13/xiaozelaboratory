...
public final class CrashUtils {
    ...
    static {
        try {
            PackageInfo pi = XZUtils.getApp()
                    .getPackageManager()
                    .getPackageInfo(XZUtils.getApp().getPackageName(), 0);
            if (pi != null) {
    ...
    public static void init(final String crashDirPath, final OnCrashListener onCrashListener) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                && XZUtils.getApp().getExternalCacheDir() != null)
            defaultDir = XZUtils.getApp().getExternalCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        else {
            defaultDir = XZUtils.getApp().getCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        }
